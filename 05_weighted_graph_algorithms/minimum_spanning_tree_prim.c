...
void minimum_spanning_tree_prim(graph *graph_ptr, int start_vertex) {
	...
	while (!is_in_tree[vertex]) {
		...
		while (tmp_edge_ptr != NULL) {
			...
			if ((cost_of_adding_to_tree[vertex_next_candidate] > current_edge_weight) && (!is_in_tree[vertex_next_candidate])) {
				cost_of_adding_to_tree[vertex_next_candidate] = current_edge_weight;
