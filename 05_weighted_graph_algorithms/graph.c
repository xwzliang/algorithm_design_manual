...
void insert_edge(graph *graph_ptr, int vertex, int vertex_connected, int edge_weight, int is_directed) {
	...
	tmp_edge_ptr->edge_weight = edge_weight;
	...
	if (is_directed)
		...
		insert_edge(graph_ptr, vertex_connected, vertex, edge_weight, 1);
}
...
void read_graph(graph *graph_ptr, int is_directed) {
	...
	for (i = 1; i <= num_edges; i++) {
		...
		insert_edge(graph_ptr, vertex_x, vertex_y, edge_weight, is_directed);
	}
}
